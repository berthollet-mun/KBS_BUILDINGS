  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
        ...
        Obx(() => DropdownButtonFormField<String>(initialValue: ctrl.formTypeDocument.value, decoration: const InputDecoration(labelText: 'Type de document'), items: ctrl.typesDocument.map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(), onChanged: (v) => ctrl.formTypeDocument.value = v ?? 'autre')),
        const SizedBox(height: 16),
        Obx(() => DropdownButtonFormField<String>(initialValue: ctrl.formEntiteType.value, decoration: const InputDecoration(labelText: 'Lié à'), items: ctrl.entiteTypes.map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(), onChanged: (v) => ctrl.formEntiteType.value = v ?? 'bien')),
